...
function MultiBot.SyncGroupAPIToPlayers()
  ...
  local playerName = type(UnitName) == "function" and UnitName("player") or nil
  local isInRaid = type(IsInRaid) == "function" and IsInRaid()
  local memberCount = 0
  if isInRaid then
    memberCount = (type(GetNumGroupMembers) == "function" and GetNumGroupMembers())
               or (type(GetNumRaidMembers) == "function" and GetNumRaidMembers()) or 0
  else
    memberCount = (type(GetNumSubgroupMembers) == "function" and GetNumSubgroupMembers())
               or (type(GetNumPartyMembers) == "function" and GetNumPartyMembers()) or 0
  end
  ...
  for i = 1, memberCount do
    ...
    local name = type(UnitName) == "function" and UnitName(unitId) or nil
    if type(name) == "string" and name ~= "" and name ~= "Unknown" and name ~= playerName then
      local classToken = nil
      if type(UnitClass) == "function" then
        local _, ct = UnitClass(unitId)
        classToken = ct
      end
      local level = (type(UnitLevel) == "function" and UnitLevel(unitId)) or 0
      local connected = type(UnitIsConnected) == "function" and UnitIsConnected(unitId) or false
      local dead = type(UnitIsDeadOrGhost) == "function" and UnitIsDeadOrGhost(unitId) or false
      local hp = (type(UnitHealth) == "function" and UnitHealth(unitId)) or 0
      local hpMax = (type(UnitHealthMax) == "function" and UnitHealthMax(unitId)) or 1
      local hpPct = hpMax > 0 and math.floor(hp * 100 / hpMax) or 0
  ...
  if type(playerName) == "string" and playerName ~= "" and type(UnitClass) == "function" then
    ...
    local playerClass = MultiBot.toClass(playerClassToken or "UNKNOWN")
    local selfButton = MultiBot.addSelf(playerClass, playerName)
    if selfButton and selfButton.setDisable then selfButton.setDisable() end
  end
    ...
    if button then
      ...
      if entry.connected then
        ...
        if previousActive[activeKey] ~= true
            ...
            and MultiBot.Comm and type(MultiBot.Comm.RequestState) == "function" then
          local stateRequest = MultiBot.Comm.RequestState(entry.name)
          if stateRequest then button.waitFor = "BRIDGE_STATE" end
        end
      else
